...
{
	pfn = (FuncType)SDL_GL_GetProcAddress(name);
}
...
{
	glVertexPointer(3, GL_FLOAT, sizeof(myVector), vertexdata);
...
void CStudioModelRenderer::SetupModelExtraData(void)
{
	...
	for (i = 0; i < m_pStudioHeader->numbodyparts; i++)
	{
		...
		for (int j = 0; j < bp[i].nummodels; j++)
		{
			...
			{
				gEngfuncs.Con_DPrintf("Warning: in model %s submodel index %d is out of range\n", m_pRenderModel->name, n);
				return;
			}
...
void CStudioModelRenderer::StudioCacheShadows()
{
	...
	while (1)
	{
		...
		if (!ent)
			break;
		if ((!ent->model) || (ent->model && ent->model->type != mod_studio))
		{
			i++;
			continue;
		}
