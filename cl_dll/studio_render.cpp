...
extern engine_studio_api_t _IEngineStudio;

#undef VectorCopy

#define VectorCopy(b, a) \
	{                    \
		(b)[0] = (a)[0]; \
		(b)[1] = (a)[1]; \
		(b)[2] = (a)[2]; \
...
void R_StudioEntityLight(studio_context_t* ctx)
{
	...
	for (int i = 0; i < 64; i++)
	{
		...
		if (elight->die <= gEngfuncs.GetClientTime())
			continue;
...
void R_StudioInitContext(studio_context_t* ctx, cl_entity_t* entity, model_t* model, studiohdr_t* header)
{
	...
	ctx->bonetransform = (mat3x4_t(*)[])_IEngineStudio.StudioGetBoneTransform();
}
...
static int CalcFxBlend(const cl_entity_t* ent)
{
	...
	case kRenderFxPulseSlow:
		amount = (int)(sinf(gEngfuncs.GetClientTime() + gEngfuncs.GetClientTime() + offset) * 16 + ent->curstate.renderamt);
		break;

	case kRenderFxPulseFast:
		amount = (int)(sinf(gEngfuncs.GetClientTime() * 8 + offset) * 16 + ent->curstate.renderamt);
		break;

	case kRenderFxPulseSlowWide:
		amount = (int)(sinf(gEngfuncs.GetClientTime() + gEngfuncs.GetClientTime() + offset) * 64 + ent->curstate.renderamt);
		break;

	case kRenderFxPulseFastWide:
		amount = (int)(sinf(gEngfuncs.GetClientTime() * 8 + offset) * 64 + ent->curstate.renderamt);
		break;
	...
	case kRenderFxStrobeSlow:
		if ((sinf(gEngfuncs.GetClientTime() * 4 + offset) * 20) >= 0)
			amount = ent->curstate.renderamt;
		else
			amount = 0;
		break;
	...
	case kRenderFxStrobeFast:
		if ((sinf(gEngfuncs.GetClientTime() * 16 + offset) * 20) >= 0)
			amount = ent->curstate.renderamt;
		else
			amount = 0;
		break;
	...
	case kRenderFxStrobeFaster:
		if ((sinf(gEngfuncs.GetClientTime() * 36 + offset) * 20) >= 0)
			amount = ent->curstate.renderamt;
		else
			amount = 0;
		break;
	...
	case kRenderFxFlickerSlow:
		if (((sinf(gEngfuncs.GetClientTime() + gEngfuncs.GetClientTime()) + sinf(gEngfuncs.GetClientTime() * 17 + offset)) * 20) >= 0)
			amount = ent->curstate.renderamt;
		else
			amount = 0;
		break;
	...
	case kRenderFxFlickerFast:
		if (((sinf(gEngfuncs.GetClientTime() * 16) + sinf(gEngfuncs.GetClientTime() * 23 + offset)) * 20) >= 0)
			amount = ent->curstate.renderamt;
		else
			amount = 0;
		break;
...
void R_StudioSetupRenderer(studio_context_t* ctx)
{
	...
	{
		chrome_origin[0] = cosf(r_glowshellfreq->value * gEngfuncs.GetClientTime()) * 4000.0f;
		chrome_origin[1] = sinf(r_glowshellfreq->value * gEngfuncs.GetClientTime()) * 4000.0f;
		chrome_origin[2] = cosf(r_glowshellfreq->value * gEngfuncs.GetClientTime() * 0.33f) * 4000.0f;
	...
	if (/* cl_righthand->value &&*/ ctx->entity == _IEngineStudio.GetViewEntity())
	...
	if (studio_gpuskin)
	{
		...
		glVertexAttribPointer(shader_studio_a_pos, 3, GL_FLOAT, GL_FALSE, sizeof(studio_gpu_vert_t), (void*)offsetof(studio_gpu_vert_t, pos));
		glVertexAttribPointer(shader_studio_a_normal, 3, GL_FLOAT, GL_FALSE, sizeof(studio_gpu_vert_t), (void*)offsetof(studio_gpu_vert_t, norm));
		glVertexAttribPointer(shader_studio_a_texcoord, 2, GL_FLOAT, GL_FALSE, sizeof(studio_gpu_vert_t), (void*)offsetof(studio_gpu_vert_t, texcoord));
		glVertexAttribPointer(shader_studio_a_bones, 2, GL_FLOAT, GL_FALSE, sizeof(studio_gpu_vert_t), (void*)offsetof(studio_gpu_vert_t, bones));
	}
	else
	{
		glEnableVertexAttribArray(shader_studio_a_pos);
		...
		glVertexAttribPointer(shader_studio_a_pos, 3, GL_FLOAT, GL_FALSE, sizeof(studio_cpu_vert_t), (void*)offsetof(studio_cpu_vert_t, pos));
		glVertexAttribPointer(shader_studio_a_normal, 3, GL_FLOAT, GL_FALSE, sizeof(studio_cpu_vert_t), (void*)offsetof(studio_cpu_vert_t, norm));
		glVertexAttribPointer(shader_studio_a_texcoord, 2, GL_FLOAT, GL_FALSE, sizeof(studio_cpu_vert_t), (void*)offsetof(studio_cpu_vert_t, texcoord));
	}
}
...
static void CalcChromeCPU(float* out, int bone_id, mat3x4_t bone, Vector normal)
{
	// cache the results like the engine does
	static int chrome_age[128];
	static Vector chrome_up[128];
	static Vector chrome_side[128];
...
void R_StudioDrawPoints(studio_context_t* ctx)
{
	...
	if (!studio_gpuskin)
	{
		...
		for (int i = 0; i < submodel->nummesh; i++)
		{
			...
			int forceflags = _IEngineStudio.GetForceFaceFlags();
