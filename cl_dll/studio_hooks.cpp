...
{
	_IEngineStudio.StudioSetHeader(header);
...
{
	_IEngineStudio.SetRenderModel(model);
...
{
	last_model = model;
	return _IEngineStudio.Mod_Extradata(model);
}
...
static model_t* DeduceModel(studiohdr_t* header)
{
	...
	gEngfuncs.Con_DPrintf("Could not find model for header %s\n", header->name);
...
static void Hk_StudioEntityLight(alight_t* lighting)
{
	...
	{
		_IEngineStudio.StudioEntityLight(lighting);
	}
}
...
static void Hk_StudioSetupLighting(alight_t* lighting)
{
	if (FASTPATH_ENABLED)
	{
		...
		R_StudioInitContext(&context, _IEngineStudio.GetCurrentEntity(), s_model, s_header);
		R_StudioSetupLighting(&context, lighting);
	}
	else
	{
		_IEngineStudio.StudioSetupLighting(lighting);
	}
}

void Hk_StudioSetupModel(int bodypart, void** ppbodypart, void** ppsubmodel)
{
	_IEngineStudio.StudioSetupModel(bodypart, ppbodypart, ppsubmodel);
...
static void Hk_StudioDrawPoints(void)
{
	...
	{
		_IEngineStudio.StudioDrawPoints();
	}
}
...
static void Hk_StudioDrawHulls(void)
{
	...
	{
		_IEngineStudio.StudioDrawHulls();
	}
}
...
static void Hk_StudioDrawAbsBBox(void)
{
	...
	{
		_IEngineStudio.StudioDrawAbsBBox();
	}
}
...
static void Hk_StudioDrawBones(void)
{
	...
	{
		_IEngineStudio.StudioDrawBones();
	}
}

static void Hk_SetupRenderer(int rendermode)
{
	_IEngineStudio.SetupRenderer(rendermode);
...
static void Hk_RestoreRenderer(void)
{
	...
	_IEngineStudio.RestoreRenderer();
}
...
void HookEngineStudio(engine_studio_api_t* studio)
{
	...
	if (studio_fastpath)
	{
		...
		studio_fastpath_toggle = gEngfuncs.pfnRegisterVariable("studio_fastpath", "1", FCVAR_ARCHIVE);
