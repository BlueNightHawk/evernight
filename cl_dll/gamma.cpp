...
{
	// check for change
	if (gammavars.brightness == v_brightness->value && gammavars.gamma == v_gamma->value && gammavars.lightgamma == v_lightgamma->value && gammavars.texgamma == v_texgamma->value)
	{
		return;
	}
...
static void UpdateGammaVars(void)
{
	...
	for (int i = 0; i < 256; i++)
	{
		...
		gammavars.textable[i] = (byte)std::clamp(color, 0, 255);

		color = (int)(255.0f * powf(f, gamma));
		gammavars.lineartable[i] = (byte)std::clamp(color, 0, 255);
	}
}